...
/*
The no arguments default LetterInventory constructor.
Parameters:
    None
*/
LetterInventory::LetterInventory() : total_count(0) {
}
...
void LetterInventory::set(char letter, int value) {
    if (isalpha(letter) == 0 || value < 0) {
        cout << "set - letter: " << letter << ", value: " << value << endl;
...
LetterInventory LetterInventory::operator+(const LetterInventory &other) {
    ...
    for (int i = 0; i < INVENTORY_LENGTH; i++) {
        newInventory.set(i + 'a', this->get(i + 'a') + other.get(i + 'a'));
    }
    return newInventory;
}
